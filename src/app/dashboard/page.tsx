...
import { Plus, Play, Sparkles, Wand2, History, ChevronRight, Loader2, Crown, Coins, Zap, MoreVertical, Video } from "lucide-react";
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div className="space-y-2">
            <div className="flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full border border-primary/20 w-fit">
               ...
               <span className="text-[10px] font-bold text-primary uppercase tracking-widest">Active Workspace</span>
            </div>
          ...
          <div className="flex items-center gap-4 bg-white/80 backdrop-blur-xl p-3 rounded-3xl border shadow-xl blue-glow">
             <div className="flex flex-col px-6 border-r border-primary/10">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-2xl font-bold font-headline">{profile?.credits ?? 0}</span>
                </div>
             </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {templates.slice(0, 4).map((template) => (
              <Link key={template.id} href={`/editor?templateId=${template.id}`} className="relative aspect-[9/16] rounded-[2.5rem] overflow-hidden group shadow-xl hover:shadow-2xl transition-all border-4 border-white">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-primary/90 via-transparent to-transparent opacity-60 group-hover:opacity-100 transition-opacity"></div>
                <div className="absolute inset-0 p-6 flex flex-col justify-end">
                  <p className="text-white text-sm font-bold leading-tight group-hover:translate-y-0 translate-y-2 transition-transform">{template.description}</p>
                </div>
              </Link>
            ))}
          </div>
