...
export function Navbar() {
  ...
  return (
    <>
      {/* DESKTOP NAV */}
      <nav className="hidden md:flex fixed top-6 left-1/2 -translate-x-1/2 z-50 glass-panel h-20 items-center justify-between px-10 rounded-full w-[95%] max-w-7xl transition-all duration-700 shadow-[0_20px_50px_rgba(0,0,0,0.8)] border-white/5">
        ...
        <div className="flex items-center gap-6">
          {profile?.isAdmin && (
            <Link href="/admin" className="text-[10px] font-black uppercase tracking-[0.3em] text-primary hover:text-white border border-primary/30 px-5 py-2 rounded-full bg-primary/5 transition-all shadow-glow">
              MASTER HUB
            </Link>
          )}
          <Link href="/premium" className="hidden lg:flex items-center gap-2 px-6 py-2.5 rounded-full bg-white text-black font-black text-[10px] uppercase tracking-widest hover:bg-primary transition-all shadow-xl">
             <Zap className="w-3.5 h-3.5 fill-current" /> Join Elite
          </Link>
        </div>
      </nav>
      ...
      {/* MOBILE NAV */}
      {!isEditor && (
        <nav className="fixed bottom-6 left-6 right-6 z-50 glass-panel md:hidden h-24 flex items-center justify-around px-4 rounded-[2.5rem] shadow-[0_30px_100px_rgba(0,0,0,0.9)] border-white/10">
          {navItems.map((item) => {
